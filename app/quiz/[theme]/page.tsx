...
export default function QuizPage() {
  ...
  if (paymentNeeded) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center px-4 py-12 md:py-16 transition-colors duration-300">
        <div className="w-full max-w-md animate-in zoom-in-95 rounded-3xl bg-white p-8 text-center shadow-xl dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
  ...
  if (missingCredits) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center px-4 py-12 md:py-16 transition-colors duration-300">
        <div className="w-full max-w-md animate-in zoom-in-95 rounded-3xl bg-white p-8 text-center shadow-xl dark:bg-slate-900 border border-red-200 dark:border-red-900/30">
      ...
      <div className="overflow-hidden rounded-3xl bg-white dark:bg-slate-900 shadow-xl shadow-slate-200/60 dark:shadow-none ring-1 ring-slate-100 dark:ring-slate-800 relative">
        <div className="bg-slate-50 dark:bg-slate-800/50 px-6 py-4 border-b border-slate-100 dark:border-slate-800">
              ...
              <div className="relative flex items-center justify-center h-12 w-12 shrink-0">
                <svg className="absolute inset-0 h-full w-full -rotate-90 transform">
                  ...
                  <circle
                    ...
                    className={`transition-all duration-1000 ease-linear ${isTimeWarning ? "text-red-500 drop-shadow-[0_0_8px_rgba(239,68,68,0.8)]" : "text-indigo-500"}`}
                  />
                </svg>
                <span className={`absolute text-sm font-black transition-colors ${isTimeWarning ? "text-red-500 animate-pulse scale-110" : "text-slate-700 dark:text-slate-200"}`}>
                  {timeLeft}
                </span>
              </div>
            )}
          ...
          <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
            <div className="h-full bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 transition-all duration-500 ease-out" style={{ width: `${progress}%` }}></div>
          </div>
        </div>
            ...
            <div key={current} className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
              ...
              <div className="grid gap-3">
                {questions[current].answers.map((ans, index) => {
                  ...
                  return (
                    <button
                      key={index} onClick={() => handleAnswer(index)} disabled={isLocked}
                      className={`group flex w-full items-center rounded-xl border-2 p-4 text-left font-semibold transition-all duration-200
                        ${isSelected ? "border-indigo-500 bg-indigo-50 text-indigo-700 ring-4 ring-indigo-500/20 scale-[0.98] dark:border-indigo-400 dark:bg-indigo-900/40 dark:text-indigo-300" 
                        : isLocked ? "border-slate-100 bg-white/50 text-slate-400 dark:border-slate-800 dark:bg-slate-800/50 dark:text-slate-600 opacity-70"
                    ...
                    >
                      <span className={`mr-4 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-xs font-bold transition-colors
                        ${isSelected ? "bg-indigo-500 text-white dark:bg-indigo-500" : "bg-slate-100 text-slate-500 group-hover:bg-indigo-100 group-hover:text-indigo-600 dark:bg-slate-700 dark:text-slate-400 dark:group-hover:bg-indigo-900/50 dark:group-hover:text-indigo-300"}`}>
            ...
            finalScore === null ? (
              <div className="flex flex-col items-center justify-center space-y-6 text-center animate-in zoom-in duration-300">
                ...
                <div className="w-full max-w-xs space-y-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 p-6 border border-slate-200 dark:border-slate-800">
                  {!session ? (
                    <input type="text" placeholder="Ton pseudo..." value={player} onChange={(e) => setPlayer(e.target.value)} className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 p-3 text-center font-bold text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all placeholder:text-slate-400" />
                  ) : (
                  ...
                  <button onClick={handleFinishQuiz} disabled={isSubmitting} className="w-full rounded-lg bg-indigo-600 py-3 font-bold text-white shadow-lg shadow-indigo-500/30 hover:bg-indigo-700 disabled:opacity-50 transition-all active:scale-95">
              ...
              <div className="flex flex-col items-center justify-center space-y-6 text-center animate-in zoom-in duration-300">
                ...
                <button onClick={() => router.push("/classement")} className="w-full max-w-xs rounded-lg bg-indigo-600 py-3 font-bold text-white shadow-lg shadow-indigo-500/30 hover:bg-indigo-700 dark:hover:bg-indigo-500 transition-all active:scale-95">
                  Voir le classement
                </button>
              </div>
            )
