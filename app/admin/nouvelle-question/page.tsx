...
export default function NewQuestionPage() {
  ...
  useEffect(() => {
    fetch("/api/themes")
      .then((res) => res.json())
      .then((data) => {
        if (Array.isArray(data)) setExistingThemes(data);
      })
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 transition-colors duration-300">
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white">Nouvelle Question</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium">Ajoutez du contenu à OnQuiz.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/50 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none sm:p-8">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Ex: Informatique, Géographie..."
        ...
        <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
          ...
          >
            {isSubmitting ? "Enregistrement en cours..." : "Sauvegarder la question"}
          </button>
        </div>
      </form>
    </div>
