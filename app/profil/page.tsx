...
export default async function ProfilePage() {
  ...
  try {
    ...
    if (users.length > 0) {
      ...
      // 2. Statistiques de jeu (On exclut mort-subite)
      const [statsResult] = await db.execute<RowDataPacket[]>(
        `SELECT 
         ...
         FROM scores WHERE user_id = ? AND theme != 'mort-subite'`,
        [userId],
      );
      ...
      // 3. Calcul de la position globale (hors mort-subite)
      const [rankResult] = await db.execute<RowDataPacket[]>(
        "SELECT COUNT(*) + 1 as globalRank FROM (SELECT SUM(score) as total FROM scores WHERE theme != 'mort-subite' GROUP BY user_id) as leaderboard WHERE total > ?",
        [totalScore],
      );

      // 4. Calcul des parties jouées aujourd'hui (Classique ET Mort Subite)
      const [todayResult] = await db.execute<RowDataPacket[]>(
        "SELECT COUNT(id) as todayGames FROM scores WHERE user_id = ? AND DATE(created_at) = CURDATE() AND theme != 'mort-subite'",
        [userId],
      );

      const [todaySDResult] = await db.execute<RowDataPacket[]>(
        "SELECT COUNT(id) as todaySDGames FROM scores WHERE user_id = ? AND DATE(created_at) = CURDATE() AND theme = 'mort-subite'",
        [userId],
      );

      // 5. Thème favori (hors mort subite)
      ...
      // 6. Record Mort Subite
      const [survivalResult] = await db.execute<RowDataPacket[]>(
        "SELECT COALESCE(MAX(score), 0) as bestStreak FROM scores WHERE user_id = ? AND theme = 'mort-subite'",
        [userId],
      );

      // 7. Points par thèmes
      const [themeStatsResult] = await db.execute<RowDataPacket[]>(
        "SELECT theme, SUM(score) as points FROM scores WHERE user_id = ? AND theme != 'mort-subite' GROUP BY theme",
        [userId],
      );
      ...
      stats = {
        ...
        freeSuddenDeathLeft: Math.max(0, 1 - todaySDResult[0].todaySDGames),
        ...
        favoriteTheme:
          favoriteThemeResult.length > 0 ? favoriteThemeResult[0].theme : "-",
        bestStreak: survivalResult[0].bestStreak,
        themeStats: themeStatsResult,
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 transition-colors duration-300">
      ...
      <div className="mb-12 flex flex-col items-center gap-8 md:flex-row md:items-center p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none relative overflow-hidden">
        <div
          className={`absolute top-0 right-0 w-64 h-64 -mr-20 -mt-20 rounded-full blur-3xl opacity-20 pointer-events-none ${userGrade.bg}`}
        ></div>
        ...
        {session.user?.image && (
          <div className="relative shrink-0">
            ...
            />
            <div
              className={`absolute -bottom-3 -right-3 flex h-10 w-10 items-center justify-center rounded-full border-4 border-white dark:border-slate-900 text-xl shadow-sm ${userGrade.bg}`}
            >
              {userGrade.icon}
            </div>
          </div>
        ...
        <div className="text-center md:text-left z-10 w-full">
          <div className="flex flex-col md:flex-row md:items-end gap-3 mb-2">
            ...
            <span
              className={`px-3 py-1 text-sm font-black uppercase tracking-wider rounded-lg ${userGrade.bg} ${userGrade.color}`}
            >
              {userGrade.title}
            </span>
          </div>
          ...
          <div className="mt-6 flex flex-wrap items-center justify-center md:justify-start gap-3">
            ...
            {/* Mort Subite Gratuite */}
            <div className="flex items-center gap-2 rounded-xl border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-900/20 px-4 py-2">
              <span className="text-lg">💀</span>
              <div>
            ...
            <div className="flex items-center gap-2 rounded-xl border border-indigo-100 dark:border-indigo-900/50 bg-indigo-50/50 dark:bg-indigo-900/20 px-4 py-2">
              <span className="text-lg">🪙</span>
              <div>
                <div className="text-xs font-bold uppercase tracking-wider text-indigo-400 dark:text-indigo-500">
                  Portefeuille
                </div>
                <div className="text-sm font-black text-indigo-700 dark:text-indigo-400">
                  {stats.credits}{" "}
                  <span className="font-medium ml-1">crédits</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <h2 className="text-xl font-black text-slate-900 dark:text-white mb-6 px-2">
        Tableau de Chasse
      </h2>
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-6 mb-12">
        ...
        <div className="col-span-1 lg:col-span-1">
          <StatCard title="Perfects" value={stats.perfectGames} icon="💯" />
        </div>
        <div className="col-span-2 lg:col-span-2">
          <StatCard
            title="Moyenne"
      ...
      {/* Maîtrise des Thèmes */}
      <div className="mb-12 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/50 dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
        ...
        <div className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
          {THEMES.map((theme) => {
            const themeStat = stats.themeStats.find(
              (s) => s.theme.toLowerCase() === theme.name.toLowerCase(),
            );
            const points = themeStat ? parseInt(themeStat.points) : 0;
            const masteryLevel = Math.min(100, (points / 100) * 100);
              ...
              <div key={theme.name} className="flex flex-col gap-2">
                <div className="flex justify-between items-center text-sm font-bold">
                  <span className="flex items-center gap-2 text-slate-700 dark:text-slate-300">
                    <span
                      className={`flex h-6 w-6 items-center justify-center rounded bg-gradient-to-br ${theme.color} text-xs text-white`}
                    >
                      {theme.icon}
                    </span>
                    {theme.name}
                ...
                <div className="h-2 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${points > 0 ? Math.max(5, masteryLevel) : 0}%`,
                    }}
                  ></div>
                </div>
              </div>
        ...
        <div className="divide-y divide-slate-50 dark:divide-slate-800">
          {stats.recentGames.length === 0 ? (
            ...
            stats.recentGames.map((game, i) => (
              <div
                key={i}
                className="flex items-center justify-between p-6 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors"
              >
                <div className="flex items-center gap-4">
                  <span
                    className={`inline-flex items-center rounded-full px-3 py-1 text-sm font-semibold ${game.theme === "mort-subite" ? "bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-300" : "bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300"}`}
                  >
                    {game.theme}
                  </span>
...
function StatCard({
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 hover:-translate-y-1 hover:shadow-md transition-all">
      ...
      <div
        className={`text-2xl font-black text-center ${highlight || "text-slate-800 dark:text-white"}`}
      >
        {value}
      </div>
      <div className="mt-2 text-xs font-bold uppercase tracking-wider text-slate-400 text-center">
        {title}
      </div>
    </div>
  );
